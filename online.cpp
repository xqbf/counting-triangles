    ...
    for(int u=0;u<n;u++){
        ...
        for(int j=0;j<len[u];j++){
            ...
            for(int k=0;k<len[v];k++){
                int w=ed[v][k];
                if(vis[w]!=-1){
                    triangle[u][j].emplace_back(std::pair<int,int>(vis[w],k));
                    triangle[u][vis[w]].emplace_back(std::pair<int,int>(j,k));
                }
            }
        }
        for(int j=0;j<len[u];j++){
...
onlineindex::onlineindex(TemporalGraph *Graph){
    ...
    triangle = new std::vector<std::vector<std::pair<int,int> > > [n]();
    point = new node[n];
    len2 = new int[n];
    len = new int[n];
