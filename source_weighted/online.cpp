...
long long onlineindex::search(TemporalGraph *Graph, int ts,int te){
    for(int i=0;i<n;i++){
        point[i].id=i;
        point[i].degree=0;
        g[i].clear();
    ...
    for(int i=0;i<n;i++){
        ...
        for(;iter!=g[u].end();iter++){
            ...
            auto k=g[v].begin();
            for(;k!=g[v].end();k++){
                if(ps[(*k).first]<ps[v])continue;
                if(g[u][(*k).first]!=0){
                    //std::cerr<<u<<' '<<v<<' '<<(*k).first<<' '<<(*k).second<<'\n';
                    ans+=g[u][(*k).first]*(*k).second*(*iter).second;
                }
            }
        }
    }
...
onlineindex::onlineindex(TemporalGraph *Graph){
    ...
    point = new node[n];
    ps = new int[n];
    g = new std::unordered_map<int,int> [n]();
    for(int i=0;i<n;i++){
        point[i].id=i;
        point[i].degree=0;
        g[i].clear();
