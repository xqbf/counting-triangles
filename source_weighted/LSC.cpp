#include "LSC.h"
#include <math.h>
bool LSCindex::cmp(mode a,mode b){
...
inline int LSCindex::find(int u,int p){
...
LSCindex::LSCindex(TemporalGraph *Graph){
    n=Graph->n;
    m=Graph->m;
    tmax=Graph->tmax;
    factor=RAND_MAX*1;
        ...
        for (it = Graph->temporal_edge[t].begin(); it != Graph->temporal_edge[t].end(); it++) {
            ...
            for(int i1=0;i1<ed[v].size();++i1){
                ...
                if(id!=-1){
                    ...
                    //tmp1=alfa[u][id];
                    //tmp2=alfa[v][i1];
                    // l1=beta[u][id].size()-1,l2=beta[v][i1].size()-1;
                    // while(l1>=0 && l2>=0){
                    //     if(beta[u][id][l1]>beta[v][i1][l2]){
                    //         if(beta[v][i1].size()-1!=l2)
                    //         if(rand()<factor)
                    //         alfa.push_back(mode(beta[u][id][l1],t,beta[v][i1].size()-l2-1));
                    //         l1--;
                    //     }
                    //     else if(beta[u][id][l1]==beta[v][i1][l2]){
                    //         if(rand()<factor)
                    //         alfa.push_back(mode(beta[u][id][l1],t,beta[v][i1].size()-l2));
                    //         l1--;
                    //     }
                    //     else{
                    //         if(beta[u][id].size()-1!=l1)
                    //         if(rand()<factor)
                    //         alfa.push_back(mode(beta[v][i1][l2],t,beta[u][id].size()-l1-1));
                    //         l2--;
                    //     }
                    //         while(l1>=0){
                    //             if(beta[u][id][l1]==beta[v][i1][l2]){
                    ...
                    //             if(beta[u][id].size()-1!=l1 && beta[u][id][l1]<beta[v][i1][l2])
                    //             if(rand()<factor)
                    //             alfa.push_back(mode(beta[v][i1][l2],t,beta[u][id].size()-l1-1));
                    //             l2--;
                    //         }
...
long long LSCindex::query1(int ts1,int ts2,int te1,int te2){
    ...
    int es2=(ts2/len)*len;
    int et2=(te2/len)*len;
    //std::cerr<<ts1<<' '<<ts2<<' '<<te1<<' '<<et1<<' '<<brute1(ts1,ts2,te1,et1)<<'\n';
    //std::cerr<<ts1<<' '<<ts2<<' '<<et2<<' '<<te2<<' '<<brute1(ts1,ts2,et2,te2)<<'\n';
    //std::cerr<<ts1<<' '<<es1<<' '<<et1+1<<' '<<et2-1<<' '<<brute1(ts1,es1,et1+1,et2-1)<<'\n';
    //std::cerr<<es2<<' '<<ts2<<' '<<et1+1<<' '<<et2-1<<' '<<brute1(es2,ts2,et1+1,et2-1)<<'\n';
    ans+=brute1(ts1,ts2,te1,et1);
    //std::cerr<<ts1<<' '<<ts2<<' '<<et2<<' '<<te2<<'\n';
    ans+=brute1(ts1,ts2,et2,te2);
    //std::cerr<<ts1<<' '<<es1<<' '<<et1+1<<' '<<et2-1<<'\n';
    ans+=brute1(ts1,es1,et1+1,et2-1);
    //std::cerr<<es2<<' '<<ts2<<' '<<et1+1<<' '<<et2-1<<'\n';
...
long long LSCindex::query2(int ts1,int ts2,int te1,int te2){
    //bigmat: mat1[ts2/len-1][te2/len-1]-mat1[(ts1-1)/len-1][te2/len-1]-mat1[ts2/len-1][(te1-1)/len-1]+mat1[(ts1-1)/len-1][(te1-1)/len-1];
    long long ans = 0;
    //std::cerr<<ts1<<' '<<ts2<<' '<<te1<<' '<<te2<<'\n';
...
void LSC(LSCindex *Index,TemporalGraph *Graph, char* query_file, char* output_file){
    int ts, te,delta;
    int query_num = 0;
    ...
    int tmp=0;
    while(fin >> ts >> te>>delta){
        double alfa=RAND_MAX;
        alfa = alfa /Index->factor;
