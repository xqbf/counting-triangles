...
measureindex::measureindex(TemporalGraph* Graph){
    ...
    for(int t=0;t<=tmax;t++){
            ...
            for(auto p:ed[v]){
                if(mp[u].count(p)){
                    ...
                    for(int i=0;i<tmp1.size();i++){
                        ...
                        cnt++;
                        expectation += (tmp2.size()-l2);
                        variance += (tmp2.size()-l2)*(tmp2.size()-l2);
                        query_expectation += 2.0*(tmax-t+1)*(tmp1[i]+1)*(tmp2.size()-l2)/(tmax+1);
                    }
                    for(int i=0;i<tmp2.size();i++){
                        ...
                        cnt++;
                        expectation += (tmp1.size()-l1);
                        variance += (tmp1.size()-l1)*(tmp1.size()-l1);
                        query_expectation += 2.0*(tmax-t+1)*(tmp2[i]+1)*(tmp1.size()-l1)/(tmax+1);
                    }
        ...
        query_expectation = query_expectation/(tmax+2);
        expectation/=cnt;
        variance/=cnt;
        std::cerr<<"Query Expectation is"<<query_expectation<<'\n';
        std::cerr<<"Expectation is "<<expectation<<'\n';
        std::cerr<<"Variance is "<<variance-expectation*expectation<<'\n';
